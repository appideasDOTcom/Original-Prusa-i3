...
// TR Nut trap
   // Hole for the nut
   /**
   *  Replace Prusa's (superior) lead screw mechanism with my printer's cheap threaded rod
   */

// Screw holes for TR nut
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) cylinder(h = 10, r = 2.9, $fn=25);

// Nut traps for TR nut screws
    translate(v=[0,-17, 0]) rotate([0, 0, -135]) translate([0, 0, 4.5]) rotate([0, 0, 0])cylinder(h = 4.5, r = 4.95, $fn=6);
